...
#var random_rotation = Vector2.RIGHT
var fire_position = Vector2.ZERO
var current_max_range = MAX_RANGE
...
func _ready() -> void:
	...
	tween.tween_method(tween_method, 0.0, 1.0, 1.0).set_ease(Tween.EASE_OUT)
	tween.tween_callback(queue_free)


func tween_method(max_range_percent):
	var player = get_tree().get_first_node_in_group("player")
	if player == null:
		return
	var percent = max_range_percent # 换算百分比
	var current_range = percent * current_max_range # 百分比*距离
